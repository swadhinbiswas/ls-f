...
# Check for Rust version first
RUST_BINARY="$SCRIPT_DIR/target/release/lsf"
USE_RUST=false

if [ -f "$RUST_BINARY" ]; then
    USE_RUST=true
elif command -v cargo >/dev/null 2>&1; then
    echo "Rust binary not found. Attempting to build with cargo..."
    if cargo build --release; then
        USE_RUST=true
    else
        echo "Warning: cargo build failed. Falling back to Bash version." >&2
    fi
fi

if [ "$USE_RUST" = true ]; then
    echo "Using Rust version (v5.0.x)"
    LSF_SOURCE="$RUST_BINARY"
else
    echo "Using legacy Bash version (v4.x)"
...
install_file() {
    ...
    if [ -e "$dest" ] && [ "$FORCE" = false ]; then
        # If running in non-interactive mode (CI), don't ask
        if [ -t 0 ]; then
            ...
            echo "Overwriting $dest (non-interactive mode)"
        fi
    fi
    cp "$src" "$dest"
    chmod +x "$dest"
...
# Install main script/binary
install_file "$LSF_SOURCE"

# Install helper (column.py) only for legacy version
if [ "$USE_RUST" = false ] && [ -f "$SCRIPT_DIR/column.py" ]; then
